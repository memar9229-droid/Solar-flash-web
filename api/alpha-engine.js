...
const CONFIG = {
  ...
  // Known Solana mint addresses — bypass DexScreener symbol search for these
  knownMints: {
    SOL:    "So11111111111111111111111111111111111111112",
    JUP:    "JUPyiwrYJFskUPiHa7hkeR8VUtAeFoSYbKedZNsDvCN",
    JTO:    "jtojtomepa8bdggrqfsgdoz4ksgwpvcdxmsqas47zk",
    WIF:    "EKpQGSJtjMFqKZ9KQanSqYXRcF8fBopzLHYxdM65zcjm",
    BONK:   "DezXAZ8z7PnrnRJjz3wXBoRgixCa6xjnB7YaB1pPB263",
    PYTH:   "HZ1JovNiVvGrCdBmqBYcpYqAbYj7PBv8dNbLxKPdbbNbZ",
    RNDR:   "rndrizKT3MK1iimdxRdqKjB1xJPFvBXr7sBGpJBJYcC",
    HNT:    "hntyVP6YFm1Hg25TN9WGLqM12b8TQmcknKrdu1oxWux",
    MOBILE: "mb1eu7TzEc71KxDpsmsKoucSSuuoGLv1drys1oP2jh6",
    OCEAN:  "99UTQE8N1sFETRnXqHxAeZzSrxzWYUNaQEagRzXbkpA5",
  },
...
async function collectAllData(mint) {
  ...
  const [rugcheck, helius, narrativeRows, smRows] = await Promise.all([
    ...
    sector ? (() => {
      // SM sectors stored as "Layer 1", "AI" etc — map from our internal ids
      const sectorDisplayName = {
        ai:"AI", depin:"DePIN", rwa:"RWA", layer1:"Layer 1",
        defi:"DeFi", gaming:"Gaming", infra:"Infrastructure",
        layer2:"Layer 2", meme:"Memecoins",
      }[sector] || sector;
      // Supabase eq filter: use ilike for case-insensitive matching
      return fetchSupabaseRow("smart_money_sectors",
        `sector=ilike.${sectorDisplayName.replace(/ /g,"%20")}&select=*&limit=1`);
...
function generateSignalLevels(direction, market, volatilityScore, requestedTimeframe) {
  ...
  // WATCH: show monitoring levels (entry zone only, wider SL, no TP targets)
  if (direction === "WATCH") {
    const price = market.price;
    const band  = price * 0.025;
    ...
    return {
      entryLow:           round(price - band),
      entryHigh:          round(price + band),
      stopLoss:           round(price - band * 3),   // wide protective SL below
      tp1:                round(price + band * 2.5),
      tp2:                round(price + band * 4.5),
      tp3:                round(price + band * 7),
      holdingPeriodHours: tfCfg.maxHoldHours,
      note:               "WATCH levels — monitor for directional confirmation",
    };
  }
...
async function resolveMintFromSymbol(symbol) {
  ...
  try {
    const j = await r.json();
    const all = (j.pairs || []).filter(p =>
      p.baseToken?.symbol?.toUpperCase() === symbol.toUpperCase() &&
      parseFloat(p.liquidity?.usd || 0) > 50000    // higher floor — avoid micro pairs
    );

    // Prefer Solana chain, then highest liquidity on any chain
    const solana = all.filter(p => p.chainId === "solana")
      .sort((a,b) => parseFloat(b.liquidity?.usd||0) - parseFloat(a.liquidity?.usd||0));
    const best = solana[0] || all.sort((a,b) =>
      parseFloat(b.liquidity?.usd||0) - parseFloat(a.liquidity?.usd||0)
    )[0];
...
// ═══════════════════════════════════════════════════════════
// SCAN MODE — analyze multiple tokens at once
// GET /api/alpha-engine?scan=true&timeframe=swing
// Returns top signals across all tracked tokens
// ═══════════════════════════════════════════════════════════

const SCAN_TOKENS = [
  { symbol:"SOL",  mint:"So11111111111111111111111111111111111111112" },
  { symbol:"JUP",  mint:"JUPyiwrYJFskUPiHa7hkeR8VUtAeFoSYbKedZNsDvCN" },
  { symbol:"JTO",  mint:"jtojtomepa8bdggrqfsgdoz4ksgwpvcdxmsqas47zk" },
  { symbol:"WIF",  mint:"EKpQGSJtjMFqKZ9KQanSqYXRcF8fBopzLHYxdM65zcjm" },
  { symbol:"BONK", mint:"DezXAZ8z7PnrnRJjz3wXBoRgixCa6xjnB7YaB1pPB263" },
  { symbol:"HNT",  mint:"hntyVP6YFm1Hg25TN9WGLqM12b8TQmcknKrdu1oxWux" },
  { symbol:"RNDR", mint:"rndrizKT3MK1iimdxRdqKjB1xJPFvBXr7sBGpJBJYcC" },
];

async function runScan(timeframe) {
  const results = await Promise.all(
    SCAN_TOKENS.map(async ({ symbol, mint }) => {
      try {
        ...
        return {
          ...
          entryZone: levels.entryLow ? [levels.entryLow, levels.entryHigh] : null,
          stopLoss:  levels.stopLoss,
          tp1:       levels.tp1,
          tp2:       levels.tp2,
          tp3:       levels.tp3,
          dataCompleteness: Math.round(data.completeness * 100),
        };
      } catch(e) {
        return { symbol, mint, direction:"IGNORE", alphaScore:0, confidence:0, error:e.message };
      }
    })
  );

  return results
    .filter(r => r.direction !== "IGNORE")
...
export default async function handler(req, res) {
  ...
  const scanMode  = req.query?.scan === "true";

  // ── SCAN MODE ──
  if (scanMode) {
    try {
      const signals = await runScan(timeframe);
      ...
      return res.status(200).json({ success:false, mode:"scan", error:e.message });
    }
  }
  ...
  if (!mint && symbol) {
    ...
    // 1. Check known mints table first (most accurate)
    if (CONFIG.knownMints[sym]) {
      mint = CONFIG.knownMints[sym];
    } else {
      // 2. Fall back to DexScreener symbol search
      const resolved = await resolveMintFromSymbol(sym);
      mint = resolved?.mint || `symbol:${sym}`;
    }
  }
