...
function SmartMoneyIntelligenceInner() {
  ...
  // Fetch real SM data from /api/smart-money/sectors
  useEffect(() => {
    fetch("/api/smart-money/sectors")
      .then(r => r.json())
      .then(d => {
          ...
          const mappedSectors = d.sectors.map(s => {
            ...
            return {
              ...
              accum:           s.accum_status === "Increasing" ? "Accumulation"
                             : s.accum_status === "Decreasing" ? "Distribution"
                             : "Neutral",
              momentum:        s.momentum,
              change7d:        parseFloat(s.change_7d || 0),
              dominance:       parseFloat(s.dominance || 0),
              activity:        s.wallet_count_active > 15 ? "Very High"
                             : s.wallet_count_active > 8  ? "High"
                             : s.wallet_count_active > 3  ? "Moderate" : "Low",
              // V2
              convictionScore:    s.conviction_score,
              capitalFlowScore:   s.capital_flow_score,
              accumHistory:       s.accum_status,
              distributionStatus: s.dist_status,
