...
const LAYERS = [
  {
    ...
    sublabel: "Layer 1 — Phase 1 Complete",
    color: C.ok,
    modules: [
      { id:"wallet",  name:"Wallet Intelligence",   icon:"⊙", desc:"Connect wallet for complete portfolio analysis. Holdings, risk exposure, smart money correlation.", status:"LIVE", href:"/dashboard", color:C.cyan   },
      { id:"market",  name:"Market Intelligence",   icon:"⚡", desc:"Real-time market monitoring. Whale activity, smart money signals, liquidity and risk alerts.", status:"LIVE", href:"/alerts",    color:C.gold   },
      { id:"token",   name:"Token Intelligence",    icon:"🎯", desc:"Complete token intelligence engine. Survival Score, Threat Radar, AI Risk Summary in one report.", status:"LIVE", href:"/token",     color:C.ok     },
    ],
  },
  {
    id: "intelligence",
    label: "INTELLIGENCE SYSTEMS",
    sublabel: "Layer 2 — Phase 2 Complete",
    color: C.cyan,
    modules: [
      { id:"narrative",name:"Narrative Radar",        icon:"📡", desc:"AI-powered trend detection. Lifecycle V2, confidence scores, rotation engine and Alpha integration.", status:"LIVE", href:"/narrative",    color:C.orange },
      { id:"smart",    name:"Smart Money Intelligence",icon:"🧠", desc:"Institutional capital intelligence. Conviction scores, accumulation history, distribution detection.", status:"LIVE", href:"/smart-money",  color:C.purple },
      { id:"opp",      name:"Opportunity Scanner",   icon:"🔍", desc:"Discovery layer. Scans all intelligence sources, ranks opportunities by score, sends best to Alpha Engine.", status:"LIVE", href:"/scanner",      color:C.cyan   },
      { id:"alpha",    name:"Alpha Engine",          icon:"⚡", desc:"Signal generation system. Takes top opportunities and generates trade-ready signals with Entry, TP, SL and AI reasoning.", status:"LIVE", href:"/alpha-engine", color:C.gold   },
      { id:"threat",   name:"Threat Scanner",        icon:"☠️", desc:"Multi-vector risk radar. Rug detection, whale risk, liquidity, volatility and contract threat analysis.", status:"LIVE", href:"/threat",       color:C.danger },
    ],
  },
  {
    id: "future",
    label: "FUTURE SYSTEMS",
    sublabel: "Layer 3 — Next Phase",
    color: C.purple,
    modules: [
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", padding:"clamp(2rem,6vw,4rem) 1rem", position:"relative" }}>
      ...
      <p style={{ fontFamily:C.body, fontSize:"clamp(1rem,2vw,1.15rem)", color:"rgba(255,255,255,.45)", letterSpacing:".08em", maxWidth:"580px", textAlign:"center", lineHeight:1.8 }}>
        Solar Flash Phase 2 is complete. From data to intelligence to opportunity to decision — every layer is live. The ecosystem spans 8 intelligence products across wallet, market, token, narrative, smart money, opportunity and signal generation.
      </p>

      {/* Stats row */}
      <div style={{ display:"flex", gap:"2rem", marginTop:"1.8rem", flexWrap:"wrap", justifyContent:"center" }}>
        {[
          { val:"8",  label:"LIVE PRODUCTS"    },
          { val:"4",  label:"FUTURE ROADMAP"   },
          { val:"2",  label:"PHASES COMPLETE"  },
...
export default function Ecosystem() {
  return (
    <div style={{ minHeight:"100vh", background:C.black, color:"#fff", fontFamily:C.font, overflowX:"hidden", position:"relative", isolation:"isolate", WebkitTapHighlightColor:"transparent" }}>
      <style>{`
      ...
      @keyframes eco-spin   { to{ transform:rotate(360deg); }  }
        ...
        @media(max-width:768px){
          .eco-modules-grid{grid-template-columns:1fr 1fr!important;}
          .eco-cta-btns{flex-direction:column!important;align-items:center!important;}
        }
        @media(max-width:480px){
          .eco-modules-grid{grid-template-columns:1fr!important;}
          .eco-core-stats-wrap{grid-template-columns:1fr 1fr!important;}
        }
      `}</style>
      ...
      <div style={{ position:"relative", zIndex:10, maxWidth:"1200px", margin:"0 auto", padding:"clamp(1.5rem,4vw,3rem) clamp(1rem,4vw,2rem)", minHeight:"100vh" }}>
        ...
        <div style={{ textAlign:"center", padding:"clamp(2rem,5vw,3.5rem) 1rem", borderTop:"1px solid rgba(255,180,0,.1)" }}>
          <div style={{ fontFamily:C.font, fontSize:".42rem", letterSpacing:".4em", color:"rgba(255,180,0,.4)", marginBottom:"1rem" }}>SOLAR FLASH — PHASE 2 COMPLETE</div>
          <h3 style={{ fontFamily:C.font, fontSize:"clamp(1rem,3vw,1.6rem)", fontWeight:900, background:"linear-gradient(135deg,#fff,#ffd700)", WebkitBackgroundClip:"text", WebkitTextFillColor:"transparent", backgroundClip:"text", letterSpacing:".1em", marginBottom:".8rem" }}>
            8 INTELLIGENCE PRODUCTS · ALL LIVE
          </h3>
          <p style={{ fontFamily:C.body, color:"rgba(255,255,255,.42)", fontSize:"1.05rem", letterSpacing:".07em", marginBottom:"1.8rem", maxWidth:"520px", margin:"0 auto 1.8rem" }}>
            From data to signal. Every intelligence layer is built and live. Phase 3 development begins next.
          </p>
          <div style={{ display:"flex", gap:"1rem", justifyContent:"center", flexWrap:"wrap" }}>
            {[
              { label:"⊙ LAUNCH APP",        href:"/app",           color:C.cyan   },
              { label:"🔍 OPPORTUNITY SCANNER",href:"/scanner",       color:C.gold   },
              { label:"⚡ ALPHA ENGINE",     href:"/alpha-engine",  color:C.orange  },
              { label:"JOIN $FLASH",         href:"https://t.me/SolarFlash_Sol", color:"rgba(255,255,255,.6)", ext:true },
            ].map((b,i) => (
