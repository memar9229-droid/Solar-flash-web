...
// ── CONFIG ───────────────────────────────────────────────────
const HELIUS_RPC = "https://mainnet.helius-rpc.com/?api-key=3ef572d9-b813-4361-bf5b-3f7a4bff3985";
...
async function fetchAll(mint) {
  const [meta, mintInfo, holders, dex] = await Promise.all([
    rpc("getAsset", { id:mint, displayOptions:{showFungible:true} }).catch(()=>null),
    rpc("getAccountInfo", [mint,{encoding:"jsonParsed"}]).then(r=>r?.value?.data?.parsed?.info).catch(()=>null),
    rpc("getTokenLargestAccounts", [mint]).then(r=>r?.value??[]).catch(()=>[]),
    fetch(`https://api.dexscreener.com/latest/dex/tokens/${mint}`).then(r=>r.json()).then(j=>j.pairs?.[0]??null).catch(()=>null),
  ]);
  return { meta, mintInfo, holders, dex };
}
...
export default function TokenIntelligence() {
  ...
  const analyze = useCallback(async () => {
    ...
    try {
      ...
      const { meta, mintInfo, holders, dex } = await fetchAll(mint);
      ...
      <div style={{ position:"relative", zIndex:10, maxWidth:"1050px", margin:"0 auto", padding:"clamp(1.5rem,4vw,3rem) clamp(1rem,4vw,2rem)" }}>
        ...
        {report && !loading && (
          <div ref={scanRef} style={{ animation:"ti-fade .5s ease" }}>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(min(100%,300px),1fr))", gap:"1.2rem", marginBottom:"1.2rem" }}>
