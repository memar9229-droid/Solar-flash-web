      ...
      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", flexWrap:"wrap", gap:"1rem" }}>
        ...
        <div style={{ display:"flex", alignItems:"center", gap:"1rem" }}>
          ...
          <div>
            ...
            <div style={{ display:"flex", alignItems:"center", gap:".6rem", marginTop:".2rem", flexWrap:"wrap" }}>
              <span style={{ fontFamily:T.body, fontSize:".9rem", color:"rgba(255,255,255,.45)" }}>
                {opp.narrative} · {opp.chain} · {signal.strategy} Strategy
              </span>
              {signal._isLive && (
                <span style={{ padding:".12rem .5rem", borderRadius:"50px", border:"1px solid rgba(80,255,160,.4)",
                  background:"rgba(80,255,160,.08)", fontFamily:T.font, fontSize:".26rem",
                  letterSpacing:".16em", color:"#50ffa0" }}>
                  ⊙ LIVE DATA
                </span>
              )}
              {signal.dataQuality && (
                <span style={{ fontFamily:T.body, fontSize:".75rem", color:"rgba(255,255,255,.3)" }}>
                  {signal.dataQuality.completeness}% data coverage
                </span>
              )}
            </div>
          </div>
  ...
  return (
    <div style={{ padding:"1.2rem 1.4rem", borderRadius:"14px", border:"1px solid rgba(0,229,255,.2)", background:"linear-gradient(135deg,rgba(0,229,255,.04),rgba(0,0,0,.55))", backdropFilter:"blur(10px)", marginBottom:"1.2rem" }}>
      ...
      <div style={{ display:"flex", flexDirection:"column", gap:".6rem" }}>
        {(Array.isArray(signal.reasoning)?signal.reasoning:Object.values(signal.reasoning||{})).map((r,i)=>(
...
export default function AlphaEngine() {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        // Load opportunity from Scanner (or use default)
        let opp;
        try {
          const stored = sessionStorage.getItem("sf_alpha_opp");
          opp = stored ? JSON.parse(stored) : DEFAULT_OPP;
        } catch {
          opp = DEFAULT_OPP;
        }
        if (!cancelled) setOpp(opp);

        // Call real Alpha Engine API
        const mint   = opp.mint;
        const symbol = opp.coin || opp.symbol;
        const tf     = "swing";

        let url;
        if (mint && mint.length >= 32 && !mint.startsWith("symbol:")) {
          url = `/api/alpha-engine?mint=${mint}&timeframe=${tf}`;
        } else if (symbol) {
          url = `/api/alpha-engine?symbol=${symbol}&timeframe=${tf}`;
        } else {
          url = `/api/alpha-engine?symbol=SOL&timeframe=${tf}`;
        }
        ...
        if (apiResult.success) {
          // Map API response to the signal format the UI expects
          const dir = apiResult.decision.direction;
          const isLong = dir === "LONG";
          const col = isLong ? T.ok : dir === "SHORT" ? T.danger : T.gold;

          const mappedSignal = {
            // Direction + meta
            direction:    dir,
            dirColor:     col,
            alphaScore:   apiResult.decision.alphaScore,
            confidence:   apiResult.decision.confidence,
            riskLevel:    apiResult.decision.riskLevel,
            timeHorizon:  apiResult.trade?.holdingPeriodHours
              ? `${apiResult.trade.holdingPeriodHours}h`
              : "24-72h",
            strategy:     Object.entries(apiResult.trade?.timeframeSuitability || {})
              .find(([,v]) => v === "Suitable")?.[0] || "Swing",

            // Levels
            entryLow:     apiResult.trade?.entryZone?.[0]  ?? null,
            entryHigh:    apiResult.trade?.entryZone?.[1]  ?? null,
            sl:           apiResult.trade?.stopLoss         ?? null,
            tp1:          apiResult.trade?.takeProfits?.tp1 ?? null,
            tp2:          apiResult.trade?.takeProfits?.tp2 ?? null,
            tp3:          apiResult.trade?.takeProfits?.tp3 ?? null,
            riskReward:   (() => {
              const entry = apiResult.trade?.entryZone?.[0];
              const tp2   = apiResult.trade?.takeProfits?.tp2;
              const sl    = apiResult.trade?.stopLoss;
              if (!entry || !tp2 || !sl || entry === sl) return "—";
              return `1 : ${Math.abs((tp2-entry)/(entry-sl)).toFixed(1)}`;
            })(),
            positionSize: "3-5% of portfolio",
            validityHours: apiResult.trade?.holdingPeriodHours ?? 72,
            ...
            timeframeSuitability: apiResult.trade?.timeframeSuitability || {},

            // Data quality
            dataQuality: apiResult.dataQuality,

            // Historical similar setups (static for V1)
            similarSetups: [
              { coin:"SOL/USDT", date:"3 weeks ago", result:"+18.4%", win:true },
              { coin:"JUP/USDT", date:"5 weeks ago", result:"+22.1%", win:true },
              { coin:"HNT/USDT", date:"8 weeks ago", result:"-4.2%",  win:false },
              { coin:"WIF/USDT", date:"10 weeks ago",result:"+12.7%", win:true },
            ],

            // Raw API for reference
            _raw: apiResult,
            _isLive: true,
          };
          setSignal(mappedSignal);
        } else {
          // API failed — fall back to simulated signal so UI stays functional
          console.warn("[AlphaEngine] API failed, falling back to simulated signal:", apiResult.error);
          setSignal(generateSignal(opp));
        }
      } catch (e) {
        console.warn("[AlphaEngine] fetch failed, using simulated signal:", e.message);
        if (!cancelled) {
          const opp = DEFAULT_OPP;
          setOpp(opp);
          setSignal(generateSignal(opp));
        }
      } finally {
